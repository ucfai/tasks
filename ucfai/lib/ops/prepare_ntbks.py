...
from ucfai.meta import meeting, Coordinator, Group, Meeting


ACCEPTED_ITEMS = set(["notebook", "post", "banner"])


def prepare_notebooks(grp: Group, auto_overwrite: bool = False) -> None:
    ...
    overhead = yaml.load(open(grp.as_dir() / "overhead.yml", "r"))
    coordinators = overhead["coordinators"]
    setattr(grp, "coords", Coordinator.parse_yaml(coordinators))

    meetings = overhead["meetings"]
    # endregion

    # region 2. Read `syllabus.yml` and parse Syllabus
    syllabus = yaml.load(open(grp.as_dir() / "syllabus.yml", "r"))
...
def _parse_and_make_meetings(key: str, group: Group, meetings: Dict, syllabus: list,
    ...
    try:
        meeting_days = overhead[key]
        syllabus = syllabus[key]
    except KeyError:
        log.warning(f"No `{key}` in the `overhead.yml` or `syllabus.yml`...")
        return

    meeting_sched = _make_schedule(group, meeting_days, offset)
...
def _prepare_post(group: Group, meeting: Meeting, auto_overwrite: bool) -> None:
    path = SITE_CONTENT_DIR / grp.as_dir(for_hugo=True) / "_posts" / repr(meeting)
    if not auto_overwrite:
        return
    ...
    ucfai = nb["metadata"]["ucfai"]

    idx = next((idx for idx, cell in enumerate(nb["cells"])
                if cell["metadata"]["type"] == "ucfai-heading"), None)
